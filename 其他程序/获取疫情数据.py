...
def get_all_today_news():
    '''获得国家卫健委的新闻'''
    print("get_all_today_news")
    url = 'http://www.nhc.gov.cn/xcs/yqtb/list_gzbd.shtml'
    r = requests.get(url, headers=quanguo_headers)
    print("r",r)
    soup = BeautifulSoup(r.text, 'lxml')
    print(soup)
    today_format ="2020-3-25" # datetime.datetime.today().strftime('%Y-%m-%d')
    ...
    print("latest_news_href",latest_news_href)
    print("get news")
...
if __name__ == "__main__":
    ...
    try:
        all_data=get_all_today_news()
        print("已获取数据 all")
        hubei_data = get_all_today_news()
        print("已获取数据")
        print('全国数据：{}\n'
